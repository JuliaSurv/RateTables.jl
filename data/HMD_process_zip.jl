...
for country_code in keys(death_rates)
    ...
    for year in minimum(availiable_years):maximum(availiable_years)
        if !(year ∈ availiable_years)
            ...
            subset_previous_year = filter(row -> row.Year == year-1, death_rates[country_code])
    ...
    rename!(lowercase, death_rates[country_code])
    death_rates[country_code] = stack(death_rates[country_code], [:male, :female, :total])
    rename!(death_rates[country_code], Dict(:variable => :sex))
end

# save files : 
for code in keys(countries)
    path = joinpath(@__DIR__, "..", "data", "HMD_data", "$(code).$(countries[code]).csv")
